...
func (r *Logger) initZap() {
	fileName := filepath.Join(r.logPath, time.Now().Format("2006-01-02")+".log")
	// 轮转文件
	...
	jsonEncoder := zapcore.NewJSONEncoder(zapcore.EncoderConfig{
		...
		EncodeTime:     zapcore.TimeEncoderOfLayout(r.timeFmt), //时间格式
	...
	consoleEncoder := zapcore.NewConsoleEncoder(zapcore.EncoderConfig{
		...
		EncodeTime:     zapcore.TimeEncoderOfLayout(r.timeFmt),
	...
	//合并两个日志核心
	r.Core = hertzzap.NewLogger(hertzzap.WithCores(consoleCore, jsonCore))
}
